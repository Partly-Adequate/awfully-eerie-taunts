...
local ic_selected = Material("vgui/taunt/ic_selected")
local ic_not_selected = Material("vgui/taunt/ic_not_selected")
...
function PANEL:Init()
	...
	self:SetZPos(-100)
	self:SetTitle("TAUNT Extension Manager")
	...
	for _, extension in ipairs(TAUNT.extensions) do
		...
		btn_extension.DoClick = function()
			...
			if extension.is_enabled then
				...
				TAUNT.EnableExtension(extension)
				ic_is_selected:SetMaterial(ic_selected)
			end
		end
	end
...
derma.DefineControl("taunt_extension_manager", "", PANEL, "DFrame")
